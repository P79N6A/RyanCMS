...
export function CustomForm(props: Props) {
  ...
  function setValidator(value: string, item: InputItemProps, options, type: Trigger) {
    const validator = item.validator;
    if (!item.formItem) {
      item.formItem = {};
    }
    if (validator && validator.trigger.includes(type)) {
      item.formItem.validateStatus = validator && validator.dataValid(value, getFormValues(options))? 'success': 'error';
    }
  }
        ...
        props.options.map((item, index) => {
          ...
          return (
            <FormItem key={index} hasFeedback={true} {...item.formItem}>
              <CustomInput {...inputProps} onChange={(e) => onChange(e, item, props.options)} onBlur = {(e) => onBlur(e, item, props.options)}/>
            </FormItem>
          )
        })
      }
